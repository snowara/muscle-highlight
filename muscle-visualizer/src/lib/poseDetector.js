 ...
 *
 * createImageBitmap(file)은 EXIF orientation을 자동 적용한 비트맵을 반환한다.
 * 모든 모던 브라우저(Chrome 50+, Firefox 42+, Safari 15+)에서 지원.
 * 미지원 시에만 수동 EXIF transform fallback을 사용한다.
 */
export async function prepareImage(file) {
  // 1차: createImageBitmap — EXIF 자동 적용, 이중 회전 위험 없음
  if (typeof createImageBitmap === "function") {
    try {
      ...
      // createImageBitmap 실패 시 fallback으로 진행
    }
  }

  // 2차 fallback: 수동 EXIF 보정 (매우 오래된 브라우저 전용)
