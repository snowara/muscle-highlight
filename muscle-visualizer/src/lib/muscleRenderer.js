...
// 색상 상수
const GOOD_COLOR = "#3B82F6"; // 파란색 - 올바른 자세
const BAD_COLOR = "#EF4444";  // 빨간색 - 잘못된 자세
...
function drawMuscleLabel(ctx, points, label, color, status, canvasW, placedLabels) {
  ...
  // pill background
  const bgColor = status === "bad" ? "rgba(239,68,68,0.2)" : "rgba(59,130,246,0.2)";
  const borderColor = status === "bad" ? "rgba(239,68,68,0.5)" : "rgba(59,130,246,0.5)";
